...
import type { ApiMethods as GrammyApiMethods, Opts as GrammyOpts } from "@grammyjs/types";
 ...
 * `InputFile` (spec §27.1) conforms to this shape structurally, so the type
 * package does not need a runtime dependency on `@telekit/core`.
 */
...
/**
 * Telekit-owned overrides for the methods whose result objects are consumed
 * directly by core. The complete registry below fills in every other current
 * Bot API method from `@grammyjs/types`; `ctx.api.raw()` remains available for
 * future or custom methods.
 */
interface TelekitOverrides {
...
/** Complete Telegram Bot API method registry, generated by @grammyjs/types. */
export type TelegramBotApiMethods = GrammyApiMethods<FileInput>;
export type TelegramBotApiOptions = GrammyOpts<FileInput>;

type GeneratedTelegramMethods = {
  [Method in keyof TelegramBotApiMethods]: {
    params: TelegramBotApiOptions[Method];
    result: ReturnType<TelegramBotApiMethods[Method]>;
  };
};

/**
 * Complete Bot API method registry. Telekit's focused object types override
 * the methods used internally, preserving the framework's backwards-
 * compatible `Update`/`Message` surface while every other current method is
 * typed from @grammyjs/types.
 */
export type TelegramMethods = Omit<GeneratedTelegramMethods, keyof TelekitOverrides> & TelekitOverrides;
